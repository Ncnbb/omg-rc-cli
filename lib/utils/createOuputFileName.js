...
function outputJSFileName(chunkhash) {
    return process.env.OMG_ENV === 'production' && chunkhash ? '[name]-[contenthash:8].js' : '[name].js';
}

function outputJSChunkFileName(chunkhash) {
    return process.env.OMG_ENV === 'production' && chunkhash ? '[name]-[contenthash:8].chunk.js' : '[name].chunk.js';
}

function outputCSSFileName(chunkhash) {
    return process.env.OMG_ENV === 'production' && chunkhash ? '[name]-[contenthash:8].css' : '[name].css';
}
function outputCSSChunkFileName(chunkhash) {
    return process.env.OMG_ENV === 'production' && chunkhash ? '[name]-[contenthash:8].chunk.css' : '[name].chunk.css';
}
