...
module.exports = class SVGLoaders {
    ...
    constructor( config ) {
        ...
        this.config = config;
        const { imageConfig, svGruntimeGenerator } = this.config;
    ...
    svgSpriteReactLoader () {
        ...
        if ( this.config.loaderOpt && this.config.loaderOpt.svg_sprite_react ) {
            const newUse = this.config.loaderOpt.scss( this.SVG_SPRITE_REACT );
            use = newUse || use;
        }
    ...
    svgSpriteLoader () {
        ...
        if ( this.config.loaderOpt && this.config.loaderOpt.svg_sprite ) {
            const newUse = this.config.loaderOpt.scss( this.SVG_SPRITE );
            use = newUse || use;
        }
    ...
    svgUrlLoader () {
        ...
        if ( this.config.loaderOpt && this.config.loaderOpt.svg_url ) {
            const newUse = this.config.loaderOpt.scss( this.SVG_URL );
            use = newUse || use;
        }
    ...
    svgInlineLoader () {
        ...
        if ( this.config.loaderOpt && this.config.loaderOpt.svg_inline ) {
            const newUse = this.config.loaderOpt.scss( this.SVG_INLIN );
            use = newUse || use;
        }
