...
function getWebpackBaseConfig ( buildPath ) {
    const config = {
        ...
        output: {
            ...
            filename: `${pathConfig.jsDir}/${process.env.OMG_ENV === 'production' ? '[name]-[chunkhash:8].js' : '[name].js'}`,
            chunkFilename: `${pathConfig.jsDir}/${process.env.OMG_ENV === 'production' ? '[name]-[chunkhash:8].chunk.js' : '[name].chunk.js'}`,
